...
void logResourceUsage(FILE *logFile, struct CPUStats *prevCpuStats, struct IOStats *prevIOStats, struct NetworkStats *prevNetStats, int interval) 
{
    ...
    float cpuUsage = (totalDiff - idleDiff) * 100.0 / totalDiff;
    if (cpuUsage > 100.0) 
    {
        cpuUsage = 100.0;
    }
    ...
    // I/O usg in KB per sec
    char ioLine[256];
    while (fgets(ioLine, sizeof(ioLine), ioFile) != NULL) 
    {
        if (strstr(ioLine, "sda")) 
        {
            break;
        }
    }
    unsigned long long read, write;
    sscanf(ioLine, "   %*u %*u sda %*u %*u %*u %*u %llu %*u %*u %*u %llu", &read, &write);
    ...
    sscanf(netLine, "enp0s3: %llu %*u %*u %*u %*u %*u %*u %*u %llu", &rec_bytes, &trans_bytes);
    ...
    fprintf(logFile, "%04d-%02d-%02d %02d:%02d:%02d,%.2f,%.2f,%.2f,%llu,%llu\n",
...
int main() 
{
    ...
    struct NetworkStats prevNetStats;
    FILE *netFile = fopen("/proc/net/dev", "r");
    if (netFile == NULL) 
    {
    ...
    while (fgets(netLine, sizeof(netLine), netFile) != NULL) 
    {
        if (strstr(netLine, "enp0s3")) 
        {
            ...
            sscanf(netLine, "enp0s3: %llu %*u %*u %*u %*u %*u %*u %*u %llu", &prevNetStats.rec_bytes, &prevNetStats.trans_bytes);
            break;
        }
    }
    fclose(netFile);
    ...
    while (fgets(ioLine, sizeof(ioLine), ioFile) != NULL) 
    {
        if (strstr(ioLine, "sda")) 
        {
            sscanf(ioLine, "   %*u %*u sda %*u %*u %*u %*u %llu %*u %*u %*u %llu", &prevIOStats.read, &prevIOStats.write);
            break;
        }
    }
    fclose(ioFile);
